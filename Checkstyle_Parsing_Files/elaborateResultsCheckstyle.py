"""
The code in this file elaborates the analysis results from monsterFileCheckstyle.csv.
...
mainPath = "/Users/lujan/Desktop/thesis_work/Maltesque2020_code-smell-prediction" \
           "/Checkstyle_Parsing_File_Outputs/monsterFileCheckstyle.csv" # Read the analysis results
...
exportDict(issueDict, 'allIssuesCheckstyle.csv')
exportDict(severitiesDict, 'allSeveritiesCheckstyle.csv')
exportDict(categoriesDict, 'allCategoriesCheckstyle.csv')
exportDict(rulesDict, 'allRulesCheckstyle.csv')

# For each of the severities, export a CSV file containing all issues under that severity.
for key in severityToIssue.keys():
    fileName = str(key) + 'IssuesCheckstyle.csv'
...
for key in categoryToRule.keys():
    fileName = str(key) + 'RulesCheckstyle.csv'
