"""
The code in this file elaborates the analysis results from monsterFileFindBugs.csv.
...
mainPath = "/Users/lujan/Desktop/thesis_work/Maltesque2020_code-smell-prediction" \
           "/FindBugs_Parsing_File_Outputs/monsterFileFindBugs.csv" # Read the analysis results
...
exportDict(issueDict, 'allIssuesFindBugs.csv')
exportDict(severitiesDict, 'allSeveritiesFindBugs.csv')
exportDict(categoriesDict, 'allCategoriesFindBugs.csv')
exportDict(rulesDict, 'allRulesFindBugs.csv')

# For each of the severities, export a CSV file containing all issues under that severity.
for key in severityToIssue.keys():
    fileName = str(key) + 'IssuesFindBugs.csv'
...
for key in categoryToRule.keys():
    fileName = str(key) + 'RulesFindBugs.csv'
